...
export function normalizeAggregateFragmentKind(kind) {
    if (kind === "thinking" || kind === "image" || kind === "video") {
        return kind;
    }
    return "content";
}
export function isAggregateTextFragment(fragment) {
    return fragment.kind === "content" || fragment.kind === "thinking";
}
export function isAggregateMediaFragment(fragment) {
    return fragment.kind === "image" || fragment.kind === "video";
}
export function aggregateFragmentSize(fragment) {
    return isAggregateTextFragment(fragment) ? fragment.text.length : 1;
}
export function normalizeAggregateFragments(fragments) {
    const normalized = [];
    for (const fragment of fragments) {
        const candidate = fragment;
        const kind = normalizeAggregateFragmentKind(typeof candidate.kind === "string" ? candidate.kind : undefined);
...
export function trimAggregateFragments(fragments, maxChars) {
    if (maxChars <= 0) {
        return [];
    }
...
export function aggregateFragmentsToText(fragments) {
    return fragments
        .filter(isAggregateTextFragment)
        .map((fragment) => fragment.text)
        .join("");
}
function normalizeAggregateMediaFragment(kind, fragment) {
...
function normalizeAggregateMediaMime(kind, mime) {
...
function normalizeAggregateDataURL(kind, url) {
...
function normalizeAggregateMetadataText(value) {
...
function buildAggregateMediaFragment(kind, url, options) {
    const fragment = { kind, url };
    if (options.mime) {
