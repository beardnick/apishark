...
export function resolveTemplate(
  input: string,
  env: Record<string, string>,
  options: RequestResolutionOptions = {},
): string {
...
function createResolutionContext(
  ...
  return {
    ...
    nowFactory:
      typeof nowOption === "function" ? (nowOption as () => Date) : () => nowOption ?? new Date(),
    nowValue: null,
  };
}

function resolveTemplateWithContext(input: string, context: ResolutionContext): string {
  if (!input) {
    return input;
  }

  const staticResolved = input.replace(STATIC_PLACEHOLDER_PATTERN, (match, key: string) => {
...
function resolveDynamicPlaceholders(input: string, context: ResolutionContext): string {
  if (!input.includes("{{$")) {
    return input;
  }
  ...
  while (cursor < input.length) {
    ...
    if (end < 0) {
      throw new RequestResolutionError(
        `Invalid dynamic placeholder starting at "${input.slice(start)}": missing closing "}}".`,
      );
    }
...
function evaluateDynamicExpression(
  token: string,
  expression: string,
  context: ResolutionContext,
): string {
  ...
  if (!callMatch) {
    throw new RequestResolutionError(
      `Unsupported dynamic placeholder ${token}. Supported forms: {{$uuid}}, {{$now}}, {{$nowMs}}, {{$isoNow}}, {{$randInt(min,max)}}, {{$randStr(len)}}, {{$base64(text)}}, {{$urlencode(text)}}.`,
    );
  }
  ...
  switch (name) {
    ...
    case "isoNow":
      if (rawArgs.trim() !== "") {
        throw new RequestResolutionError(
          `Invalid dynamic placeholder ${token}: ${name} does not accept arguments.`,
        );
      }
    ...
    default:
      throw new RequestResolutionError(
        `Unsupported dynamic placeholder ${token}. Function "${name}" is not allowed.`,
      );
  }
}

function resolveRandInt(token: string, rawArgs: string, context: ResolutionContext): string {
  const parts = rawArgs.split(",");
  if (parts.length !== 2) {
    throw new RequestResolutionError(
      `Invalid dynamic placeholder ${token}: randInt(min,max) expects two integer arguments.`,
    );
  }
  ...
  if (min > max) {
    throw new RequestResolutionError(
      `Invalid dynamic placeholder ${token}: randInt(min,max) requires min <= max.`,
    );
  }
...
function resolveRandStr(token: string, rawArgs: string, context: ResolutionContext): string {
  const length = parseIntegerArg(rawArgs, token, "randStr len");
  if (length < 0) {
    throw new RequestResolutionError(
      `Invalid dynamic placeholder ${token}: randStr(len) requires len >= 0.`,
    );
  }
...
function parseIntegerArg(rawValue: string, token: string, label: string): number {
  const value = rawValue.trim();
  if (!/^-?\d+$/.test(value)) {
    throw new RequestResolutionError(
      `Invalid dynamic placeholder ${token}: ${label} must be an integer.`,
    );
  }
  return Number.parseInt(value, 10);
}

function getNow(context: ResolutionContext): Date {
  if (context.nowValue) {
...
function encodeBase64Utf8(input: string): string {
  ...
  const buffer = (globalThis as typeof globalThis & { Buffer?: NodeLikeBuffer }).Buffer;
  if (buffer) {
